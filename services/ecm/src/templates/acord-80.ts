...
export async function generateAcord80(data: Acord80Data): Promise<Uint8Array> {
  ...
  y = drawSectionHeader(page, y, "Property Location", fonts);
  y = drawFieldRow(page, y, [
    { label: "STREET ADDRESS", value: client.address.street },
    { label: "CITY", value: client.address.city },
  ], fonts);
  y = drawFieldRow(page, y, [
    { label: "STATE", value: client.address.state },
    { label: "ZIP CODE", value: client.address.zip },
  ], fonts);
  ...
  y = drawSectionHeader(page, y, "Homeowners Coverage Schedule", fonts);
  y = drawCoverageTable(page, y, policy.coverages, fonts);

  // Footer
  drawFooter(page, "80", fonts);
